...
describe('FleteFormComponent', () => {
  ...
  const createRouter = {
    ...router, url: `flete/${m.FLETE}/${a.CREAR}`,
  }
  const editRouter = {
    ...router, url: `flete/${m.FLETE}/${a.EDITAR}/:id`,
  }
  const showRouter = {
    ...router, url: `flete/${m.FLETE}/${a.VER}`,
  }
  ...
  function formSetValue(component: FleteFormComponent): void {
    component.form.patchValue({
      ...
      condicion: {
        ...
        condicion_gestor_cuenta_moneda_id: mockFlete1.condicion_gestor_cuenta_moneda_id,
        condicion_gestor_cuenta_tarifa: mockFlete1.condicion_gestor_cuenta_tarifa,
        condicion_gestor_cuenta_unidad_id: mockFlete1.condicion_gestor_cuenta_unidad_id,
        // fin - Condiciones para el Gestor de Cuenta
        // inicio - Condiciones para el Propietario
        condicion_propietario_moneda_id: mockFlete1.condicion_propietario_moneda_id,
        condicion_propietario_tarifa: mockFlete1.condicion_propietario_tarifa,
        condicion_propietario_unidad_id: mockFlete1.condicion_propietario_unidad_id,
      ...
      merma: {
        ...
        merma_gestor_cuenta_es_porcentual: mockFlete1.merma_gestor_cuenta_es_porcentual,
        merma_gestor_cuenta_tolerancia: mockFlete1.merma_gestor_cuenta_tolerancia,
        ...
        merma_propietario_es_porcentual: mockFlete1.merma_propietario_es_porcentual,
    ...
    await TestBed.configureTestingModule({
      imports: [
        ...
        RouterTestingModule.withRoutes([
          { path: `flete/${m.FLETE}/${a.CREAR}`, component: FleteFormComponent },
          { path: `flete/${m.FLETE}/${a.EDITAR}`, component: FleteFormComponent },
  ...
  it('should open create view', fakeAsync(() => {
    ...
    httpController.match(`${environment.api}/remitente/gestor_cuenta_id/`).forEach(r => r.flush(mockRemitenteList));
    httpController.match(`${environment.api}/centro_operativo/gestor_cuenta_id/`).forEach(r => r.flush(mockCentroOperativoList));
    httpController.match(`${environment.api}/producto/`).forEach(r => r.flush(mockProductoList));
    ...
    httpController.match(`${environment.api}/flete/destinatarios/${mockFlete1.remitente_id}/${mockFlete1.origen_id}/${mockFlete1.destino_id}`).forEach(r => r.flush(mockFleteDestinatarioList));
  ...
  it('should open edit view', fakeAsync(() => {
    ...
    const backSpy = spyOn(component, 'back').and.callThrough();
    fixture.detectChanges();
    httpController.match(`${environment.api}/flete/${id}`).forEach(r => r.flush(mockFlete1));
    httpController.match(`${environment.api}/remitente/gestor_cuenta_id/`).forEach(r => r.flush(mockRemitenteList));
    httpController.match(`${environment.api}/centro_operativo/gestor_cuenta_id/`).forEach(r => r.flush(mockCentroOperativoList));
    httpController.match(`${environment.api}/producto/`).forEach(r => r.flush(mockProductoList));
    httpController.match(`${environment.api}/tipo_carga/`).forEach(r => r.flush(mockTipoCargaList));
  ...
  it('should open edit view with alias null', fakeAsync(() => {
    ...
    fixture.detectChanges();
    pageFormComponent = findElement(fixture, 'app-page-form');
    httpController.match(`${environment.api}/flete/${id}`).forEach(r => r.flush(mockFlete2));
    httpController.match(`${environment.api}/remitente/gestor_cuenta_id/`).forEach(r => r.flush(mockRemitenteList));
    httpController.match(`${environment.api}/centro_operativo/gestor_cuenta_id/`).forEach(r => r.flush(mockCentroOperativoList));
    httpController.match(`${environment.api}/producto/`).forEach(r => r.flush(mockProductoList));
    httpController.match(`${environment.api}/tipo_carga/`).forEach(r => r.flush(mockTipoCargaList));
    ...
    httpController.match(`${environment.api}/tipo_concepto_complemento/`).forEach(r => r.flush(mockTipoConceptoComplementoList));
    httpController.match(`${environment.api}/tipo_concepto_descuento/`).forEach(r => r.flush(mockTipoConceptoDescuentoList));
    httpController.match(`${environment.api}/flete/destinatarios/${mockFlete2.remitente_id}/${mockFlete2.origen_id}/${mockFlete2.destino_id}`).forEach(r => r.flush(mockFleteDestinatarioList));
    flush();
    tick();
    expect(getByIdSpy).toHaveBeenCalled();
    tick();
    ...
    httpController.match(`${environment.api}/flete/${id}/cancel`).forEach(r => r.flush(mockFlete2));
    flush();
    tick(1);
