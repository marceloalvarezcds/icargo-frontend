...
export const findPermiso = (
  user: UserAccount,
  accion: PermisoAccionEnum,
  modelo: PermisoModeloEnum
): boolean => {
  return (
    user.permisos.findIndex(
      (p) => p.accion === accion && p.modelo === modelo
    ) !== -1
  );
};
...
export const checkPermisoAndGestorCargaId = (
  user: UserAccount | null,
  accion: PermisoAccionEnum,
  modelo: PermisoModeloEnum,
  ...
  return (
    findPermiso(user, accion, modelo) && user.gestor_carga_id === gestorCuentaId
  );
};
