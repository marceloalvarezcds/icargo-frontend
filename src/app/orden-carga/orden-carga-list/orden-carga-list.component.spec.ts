...
import {
  mockOrdenCargaList,
  OrdenCargaList,
} from 'src/app/interfaces/orden-carga';
...
describe('OrdenCargaListComponent', () => {
  ...
  it('listens for app-table changes', fakeAsync(() => {
    ...
    httpController
      .expectOne(`${environment.api}/orden_carga/`)
      .flush(mockOrdenCargaList);
    const req = httpController.expectOne(
      `${environment.api}/orden_carga/${row.id}`
    );
    expect(req.request.method).toBe('DELETE');
    req.flush({});
    flush();
    httpController
      .match(`${environment.api}/orden_carga/`)
      .forEach((r) => r.flush(mockOrdenCargaList));
    flush();
