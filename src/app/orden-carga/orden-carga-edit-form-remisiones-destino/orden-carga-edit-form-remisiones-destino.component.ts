...
export class OrdenCargaEditFormRemisionesDestinoComponent {
  ...
  remove({ row }: TableEvent<OrdenCargaRemisionDestino>): void {
    remove(this.dialog, `¿Está seguro que desea eliminar a la remisión de destino ${row.numero_documento}?`, () => {
      this.ordenCargaRemisionDestinoService.delete(row.id).subscribe(this.emitOcChange.bind(this));
    });
  }

  private getDialogRef(item?: OrdenCargaRemisionDestino): MatDialogRef<OcRemisionDestinoFormDialogComponent, OrdenCargaRemisionDestino> {
