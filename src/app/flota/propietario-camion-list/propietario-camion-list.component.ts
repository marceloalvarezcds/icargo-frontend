...
export class PropietarioCamionListComponent {
  ...
  redirectToCreate(): void {
    this.router.navigate([`/flota/${m.CAMION}/${a.CREAR}`], { queryParams: { backUrl: this.backUrl, propietarioId: this.id } });
  }

  redirectToEdit(event: TableEvent<CamionList>): void {
    this.router.navigate([`/flota/${m.CAMION}/${a.EDITAR}`, event.row.id], { queryParams: { backUrl: this.backUrl, propietarioId: this.id }});
  }

  redirectToShow(event: TableEvent<CamionList>): void {
    this.router.navigate([`/flota/${m.CAMION}/${a.VER}`, event.row.id], { queryParams: { backUrl: this.backUrl }});
  }
