...
export class CombinacionFormInfoComponent implements AfterViewInit, OnInit {
  ...
  camionChange(camion?: CamionList){
    this.info?.controls["placa"].setValue(camion?.placa)
  ...
  tipoPersonaChange(propietario?: PropietarioList){
    this.info?.controls["tipo_persona"].setValue(propietario?.tipo_persona.descripcion)
    this.info?.controls["nombre"].setValue(propietario?.nombre)
    this.info?.controls["ruc"].setValue(propietario?.ruc)
    this.info?.controls["cedula"].setValue(propietario?.numero_documento)
