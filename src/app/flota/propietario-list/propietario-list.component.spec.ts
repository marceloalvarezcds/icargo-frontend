...
import {
  mockPropietarioList,
  PropietarioList,
} from 'src/app/interfaces/propietario';
...
describe('PropietarioListComponent', () => {
  ...
  it('listens for app-table changes', fakeAsync(() => {
    ...
    httpController
      .expectOne(`${environment.api}/propietario/`)
      .flush(mockPropietarioList);
    const req = httpController.expectOne(
      `${environment.api}/propietario/${row.id}`
    );
  ...
  it('should apply filter', fakeAsync(() => {
    ...
    const tipoPersonaFiltered = component.tipoPersonaFilterList.filter(
      (_, i) => i === 0
    );
    ...
    spyOn(
      component.tipoPersonaCheckboxFilter,
      'getFilteredList'
    ).and.returnValue(tipoPersonaFiltered);
