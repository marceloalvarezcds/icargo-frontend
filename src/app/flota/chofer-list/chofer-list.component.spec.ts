...
import { ChoferList, mockChoferList } from 'src/app/interfaces/chofer';
...
describe('ChoferListComponent', () => {
  ...
  const row = mockChoferList[0];
  const tableEvent: TableEvent<ChoferList> = {
    row, index: 0,
  }
    ...
    await TestBed.configureTestingModule({
      imports: [
        ...
        RouterTestingModule.withRoutes([
          ...
          {
            path: `flota/${m.CHOFER}/${a.EDITAR}/:id`,
            component: ChoferFormComponent,
          },
          {
            path: `flota/${m.CHOFER}/${a.VER}/:id`,
            component: ChoferFormComponent,
          },
        ]),
        SharedModule,
      ...
      declarations: [ ChoferListComponent, ChoferFormComponent ],
    })
    .compileComponents();
  });
  ...
  it('listens for app-table changes', fakeAsync(() => {
    ...
    httpController.expectOne(`${environment.api}/chofer/`).flush(mockChoferList);
    const req = httpController.expectOne(`${environment.api}/chofer/${row.id}`)
  ...
  it('should make an http call to download the list', fakeAsync(() => {
    ...
    const filename = 'chofer_reports.xls';
    ...
    httpController.expectOne(`${environment.api}/chofer/reports/`).flush(filename);
  ...
  it('should apply filter', fakeAsync(() => {
    ...
    const chofer = mockChoferList.find((_, i) => i === 0)!;
    component.filterPredicate(chofer, filterStr);
    component.filterPredicate(chofer, '{}');
    component.columns.forEach(c => c.value && c.value(chofer));
    httpController.verify();
  }));
});
