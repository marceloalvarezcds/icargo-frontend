...
export class PropietarioSemiListComponent {
  ...
  redirectToCreate(): void {
    this.router.navigate([`/flota/${m.SEMIRREMOLQUE}/${a.CREAR}`], { queryParams: { backUrl: this.backUrl, propietarioId: this.id } });
  }

  redirectToEdit(event: TableEvent<SemiList>): void {
    this.router.navigate([`/flota/${m.SEMIRREMOLQUE}/${a.EDITAR}`, event.row.id], { queryParams: { backUrl: this.backUrl, propietarioId: this.id }});
  }

  redirectToShow(event: TableEvent<SemiList>): void {
    this.router.navigate([`/flota/${m.SEMIRREMOLQUE}/${a.VER}`, event.row.id], { queryParams: { backUrl: this.backUrl }});
  }
  ...
  deleteRow(event: TableEvent<SemiList>): void {
    ...
    confirmationDialogToDelete(this.dialog, message, this.semiService, row.id, this.snackbar, {
      next: () => { this.getList(); }
    });
  }
