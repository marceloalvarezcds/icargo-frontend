...
export class OcComplementoFormDialogComponent {
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      if (this.data?.id) {
        this.ordenCargaComplementoService.edit(this.data?.id, formData).subscribe(this.close.bind(this));
      } else {
        this.ordenCargaComplementoService.create(formData).subscribe(this.close.bind(this));
      }
    }
  }
