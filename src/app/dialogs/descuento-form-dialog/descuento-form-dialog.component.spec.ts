...
describe('DescuentoFormDialogComponent', () => {
  ...
  const mockDialogRefSpyObj = jasmine.createSpyObj({ close : (data?: FleteDescuento) => {} });
  ...
  it('data should be null and should submitted', fakeAsync(() => {
    ...
    httpController.match(`${environment.api}/proveedor/gestor_cuenta_id/`).forEach(r => r.flush(mockProveedorList));
    button.click();
    tick();
    expect(component.form.valid).toBeTruthy();
    expect(submitSpy).toHaveBeenCalled();
