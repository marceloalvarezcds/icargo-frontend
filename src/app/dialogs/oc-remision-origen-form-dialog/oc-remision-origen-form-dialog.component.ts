...
export class OcRemisionOrigenFormDialogComponent {
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      if (this.data?.id) {
        this.ordenCargaRemisionOrigenService.edit(this.data?.id, formData).subscribe(this.close.bind(this));
      } else {
        this.ordenCargaRemisionOrigenService.create(formData).subscribe(this.close.bind(this));
      }
    }
  }
