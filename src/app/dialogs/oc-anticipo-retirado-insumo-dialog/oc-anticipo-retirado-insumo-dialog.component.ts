...
get montoRetiradoHint(): string {
  ...
  // Saldo restante del tracto después de retirar el monto
  const saldoTractoRestante = (this.anticipoDisponibleCamion ?? 0) - monto;

  // El saldo a mostrar es el mínimo entre saldo disponible y saldo restante del tracto
  const saldoMostrar =
  ...
  if (saldoMostrar < 0) {
    const excedente = formatNumber(monto - Math.min(this.saldoDisponible, this.anticipoDisponibleCamion ?? 0));
    return `<span>El saldo es negativo: <strong>${formatNumber(saldoMostrar)}</strong>. El monto supera en <strong>${excedente}</strong> al saldo.</span>`;
  }

  if (this.limiteAnticipoCamion !== null && this.limiteAnticipoCamion > 0) {
    if (monto > this.saldoDisponible) {
      return `<span class="hint-alert">El monto supera en <strong>${formatNumber(monto - this.saldoDisponible)}</strong> al saldo disponible</span>`;
    }

    if (monto > (this.anticipoDisponibleCamion ?? 0)) {
      return `<span class="hint-alert">El monto supera en <strong>${formatNumber(monto - (this.anticipoDisponibleCamion ?? 0))}</strong> al anticipo del tracto disponible</span>`;
    }
  }
